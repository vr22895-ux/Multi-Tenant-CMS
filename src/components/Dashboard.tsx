...
import { createPost, getCompanies, createCompany, parseUploadedFile, getPosts } from '@/app/actions';
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (result.success && result.data && result.data.length > 0) {
            if (result.isCsv) {
                ...
                setSlug(result.data[0].title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, ''));
                setContent(result.data[0].content);
                setMessage({ type: 'success', text: 'Document loaded into editor!' });
            }
            refreshData();
        } else {
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div className="space-y-4">
                                            ...
                                            <div className="pt-2">
                                                ...
                                                <p className="text-xs text-gray-500 mt-2 ml-1">CSV runs a bulk-upload batch. Docs auto-fill the editor.</p>
                                            </div>
                                        </div>
                                    ...
                                    posts.map(post => (
                                        <li key={post.id} className="p-4 hover:bg-gray-50 transition-colors">
                                            <div className="flex justify-between items-start mb-1">
                                                ...
                                                <span className="shrink-0 text-[10px] bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full font-bold uppercase tracking-wide ml-2">
                                                    {post.type}
                                                </span>
                                            </div>
